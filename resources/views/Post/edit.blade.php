...
@section('content')
    <div class="flex flex-col items-center w-full h-full ">
        <div class=" flex w-full justify-center mb-3">
            ...
            <h2 class="font-bold text-2xl text-yellow-400">Update your post</h2>
        </div>
        <form action={{route('posts.update',$post->id)}} method="post"
            ...
            <div class="flex flex-col space-y-1">
                ...
                <textarea class="border-3 p-2 w-4/5 rounded-2xl max-h-48" name="content" id="content"
                          placeholder="Input main content"  >{{$post->content}}</textarea>
                @error('content')
                <p class="text-red-600"> {{$message}}</p>
                @enderror
            </div>
            ...
            <div class="flex flex-col space-y-1">
                ...
                <input class="border-3 w-4/5 p-2 rounded-2xl" type="text" name=image"" id="image"
                       placeholder="Input image url" value="{{$post->image}}">
                @error('image')
                <p class="text-red-600"> {{$message}}</p>
                @enderror
            </div>
